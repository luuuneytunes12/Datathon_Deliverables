...
text_data = [
    doc for doc in text_data if len(doc.split()) > 3
]  # Keep only documents with at least 3 words

# Ensure text_data is not empty before vectorization
if not text_data:
    raise ValueError(
        "Error: After preprocessing, there are no valid documents left. Check your dataset!"
    )

# 1️⃣ Use BERTopic with a modified vectorizer
vectorizer = CountVectorizer(
    stop_words="english", token_pattern=r"(?u)\b\w+\b"
)  # Allows words + numbers
bertopic_model = BERTopic(
    vectorizer_model=vectorizer, nr_topics=10, umap_model=UMAP(random_state=42)
)  # Set random_state in UMAP
...
custom_labels = {
    ...
    for topic, name in zip(
        topic_info["Topic"].tolist(), topic_info["Cleaned Name"].tolist()
    )
}
...
topic_embeddings = model.encode(
    [custom_labels.get(topic, f"Topic {topic}") for topic in bertopic_topics]
)

# Debug: Check if embeddings are too similar
np.set_printoptions(
    precision=8, suppress=True
)  # Fix floating-point precision inconsistencies
