...
# Load the data
df = pd.read_csv("C:/Users/Admin/combined_articles.csv", encoding="latin1", sep=",")
df.columns = ["Text"]
...
for n in range(len(df["Text"])):
    ...
    for sentence in doc.sents:
        topics = [token.text.lower() for token in sentence if token.dep_ in ["nsubj", "dobj", "pobj"]]
        for t in topics:
            topiclist.append(t)
...
for key,value in topicdict.items():
    l= []
    uselesswords = ["he", "they", "all", "them", "this", "it", "she", "her", "him","may","that","who"
                    ,"you","our","no", "yes", "non","which","when", "am","pm","we",
                    "monday","wednesday", "tuesday", "thursday", "friday", "saturday", "sunday",
                    "jan","feb", "mar","apr","may","jun","jul","aug","sep","oct","nov","dec",
                    "january","febuary", "march","april","may","june","july", "august","september","october","november","december",
                    "reuters","time", "date","case","people"]
    for item in value: #filtering for repeats and filler words
        if item not in l and item not in uselesswords and item.isalpha() and len(item)>1:
            l.append(item)
    st = " ".join(l)
    topicdict[key] =[st]
topicdf = pd.DataFrame(topicdict)
topicdf.to_csv("topics.csv")


topicscsv = pd.read_csv("C:/Users/Admin/topics.csv",header=None)
dfv=topicscsv.T
dfv.to_csv("topics_formatted.csv",index=False,header=False)
#-----------------------------------------------------------------
...
def extract_topics(articles, num_topics=5, top_n_words=10):
    ...
    for topic_idx, topic in enumerate(lda.components_):
        top_keywords = [vectorizer.get_feature_names_out()[i] for i in topic.argsort()[-top_n_words:]]
...
similarity_df = pd.DataFrame(similarity_matrix, index=[f"Topic {i+1}" for i in range(len(tl))],
...
sorted_degrees = sorted(node_degrees.items(), key=lambda x: x[1], reverse=True)  # Sort by degree in descending order
